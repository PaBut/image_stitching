...
def make_matching_figure(
    ...
    fig, axes = plt.subplots(1, 2, figsize=(15, 9), dpi=dpi)
    axes[0].imshow(cv2.cvtColor(img0.astype(np.uint8), cv2.COLOR_BGR2RGB))
    axes[1].imshow(cv2.cvtColor(img1.astype(np.uint8), cv2.COLOR_BGR2RGB))
    ...
    mask = ((mkpts0[:, 0] >= 0) & (mkpts0[:, 0] < img0.shape[1]) & (mkpts0[:, 1] >= 1) & (mkpts0[:, 1] < img0.shape[0]) 
    & (mkpts1[:, 0] >= 0) & (mkpts1[:, 0] < img1.shape[1]) & (mkpts1[:, 1] >= 1) & (mkpts1[:, 1] < img1.shape[0]))
